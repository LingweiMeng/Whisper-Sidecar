# Reference: https://github.com/JusperLee/Conv-TasNet/blob/master/Conv_TasNet_Pytorch/Conv_TasNet.py
...
class Sidecar(nn.Module):
    ...
    def forward(self, x):
        ...
        m = self.gen_masks(e)
        m = self.activation(m)
        # n x num_spk*N x T => n x num_spk x N x T
        m = m.view(batch_size, self.num_spks, N, T)

        # n x num_spk x N x T => n*num_spk x N x T
        d = w.unsqueeze(1) * m
        d = d.view(batch_size*self.num_spks, N, T)
